//! # fsim-control
//!
//! The autopilot. At its core are the two innermost loops of the cascade:
//!
//! ```text
...
//! position/guidance loops wrap around these in the INS; LQR/MPC can replace the
...
/// move onto the threaded engine's worker thread. `Send` is a `core` marker, so the
